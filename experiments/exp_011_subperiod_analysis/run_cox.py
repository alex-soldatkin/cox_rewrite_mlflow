...
def create_crisis_indicators(df, crisis_config):
    """
    Create crisis period indicator variables.
    
    Args:
        df: DataFrame with 'DT' column (datetime)
        crisis_config: Dictionary with crisis period definitions
        
    Returns:
        DataFrame with crisis indicator columns
    """
    ...
    for crisis_name, crisis_info in crisis_config.items():
        start_date = pd.to_datetime(crisis_info['start'])
        end_date = pd.to_datetime(crisis_info['end'])
        
        df[crisis_name] = (
            (df['DT'] >= start_date) & (df['DT'] <= end_date)
        ).astype(int)
        
        crisis_indicators.append(crisis_name)
        
        n_obs = df[crisis_name].sum()
        pct = 100 * df[crisis_name].mean()
        print(f"  {crisis_name}: {n_obs:,} obs ({pct:.1f}%)")
        print(f"    Period: {start_date.date()} to {end_date.date()}")
        print(f"    Type: {crisis_info['type']}")
    
    # Combined crisis indicator
    df['crisis_any'] = df[crisis_indicators].max(axis=1)
    n_crisis_any = df['crisis_any'].sum()
    pct_any = 100 * df['crisis_any'].mean()
    print(f"  crisis_any: {n_crisis_any:,} obs ({pct_any:.1f}%)")
    
    return df, crisis_indicators

def create_interaction_terms(df, features, crisis_indicators):
    """
...
def main():
    ...
    # 5. Create Crisis Indicators
    print(f"\n3. Creating crisis indicators...")
    df, crisis_indicators = create_crisis_indicators(df, crisis_config)
    ...
    for model_idx, (model_key, model_cfg) in enumerate(models_config.items(), 1):
        ...
        # Add crisis dummies if requested
        if model_cfg.get('include_crisis_dummies', False):
            model_features.extend(crisis_indicators)
            print(f"\n  Adding crisis dummies: {crisis_indicators}")
        
        # Create interaction terms if requested
        interaction_vars = model_cfg.get('interaction_features', [])
        if interaction_vars:
            df_model = df.copy()
            df_model, interaction_features = create_interaction_terms(
                df_model, 
                interaction_vars, 
                crisis_indicators
            )
            model_features.extend(interaction_features)
        else:
            df_model = df.copy()
        ...
        run_model(
            ...
            model_key=model_key,
            lag_quarters=data_config['lag_quarters'],
            has_crisis_dummies=model_cfg.get('include_crisis_dummies', False),
            has_interactions=len(interaction_vars) > 0
        )
